...
const COLORS = ["var(--accent-green)", "var(--accent-gold)", "var(--chart-4)", "var(--chart-5)", "var(--accent-red)", "#9ca3af"];

export function DailyPnLChart({ points, tipsters }: { points: DailyTipsterPoint[]; tipsters: string[] }) {
  return (
        ...
        {points.length === 0 ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={points} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
              ...
              <XAxis dataKey="date" stroke="var(--color-muted-foreground)" fontSize={11} tickFormatter={(v: string) => v.slice(5)} />
              <YAxis stroke="var(--color-muted-foreground)" fontSize={11} tickFormatter={(v: number) => `${v}€`} width={50} />
              <Tooltip
