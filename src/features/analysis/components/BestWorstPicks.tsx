  ...
  return (
    <div className="rounded-xl border border-border bg-card">
      ...
      <ul className="divide-y divide-border">
        {bets.length === 0 && (
          <li className="px-5 py-6 text-center text-sm text-muted-foreground">Sin datos</li>
        )}
        {bets.map((b) => (
          <li key={b.id} className="flex items-center gap-3 px-5 py-3 text-sm">
            <div className="min-w-0 flex-1">
              ...
              <div className="text-xs text-muted-foreground">
                {fDate(b.bet_date)} · {b.tipster} · {b.market}
              </div>
            </div>
            <div className="font-mono-num text-xs text-muted-foreground">{fOdds(b.odds)}</div>
            <div className={`w-20 text-right font-mono-num font-medium ${pnlClass(b.pnl)}`}>
              {fEUR(Number(b.pnl ?? 0))}
            </div>
          </li>
        ))}
