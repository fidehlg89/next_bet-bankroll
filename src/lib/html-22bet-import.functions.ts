...
/**
 * Parses the HTML exported from 22Bet's bet history page and upserts each
 * ticket as a bet row for the authenticated user.
 *
 * - Tipster is set to "Sin asignar"
 * - Stake is kept as-is (EUR)
 * - Duplicates are matched by external_id ("22bet:<ticket>"):
 *     · same ticket already resolved → skipped
 *     · same ticket still pending in DB, new result available → updated
 *     · not found → inserted
 */
...
/**
 * Validates that the pasted text looks like a 22Bet bet-history export.
 * Throws an Error with a clear, user-facing message if it doesn't.
 */
export function validate22BetHtml(html: string): void {
  ...
  if (!hasBlocks) {
    throw new Error(
      "Formato de 22Bet inválido: no se encontraron bloques de boletos (cupHisNew).",
    );
  }
  if (!hasTicket) {
    throw new Error(
...
export function parse22BetHtml(html: string): ParsedBet[] {
  const out: ParsedBet[] = [];
  // Each ticket block starts at <div class="cupHisNew ...">. Split by that marker.
  const blocks = html.split(/<div class="cupHisNew[^"]*">/g).slice(1);

  for (const rawBlock of blocks) {
    // Cut at the closing of the wrapper – it always ends with </table>\n            </div>\n</div>
    const endIdx = rawBlock.indexOf("</table>");
    const block = endIdx >= 0 ? rawBlock.slice(0, endIdx + 8) : rawBlock;

    const isExpress = /class="cupHisNew express"/.test(html) // (kept for safety) – real check below:
      ? true
      : false;
    void isExpress;
    ...
    const bet_type: "Simple" | "Combinada" = typeMatch && /combinada/i.test(typeMatch[1])
      ? "Combinada"
      : "Simple";

    // Sport + event come from the first leg row (td.ha > span > b)
    const legRegex =
    ...
    while ((lm = legRegex.exec(block)) !== null) {
      ...
      // raw looks like "Fútbol. Copa Mundial 2026 Equipo A - Equipo B"
      // The <br> separates sport.league from event. After stripTags both are joined by a space.
      // Re-split using the <br>:
      const rawHtml = lm[1];
    ...
    const event =
      bet_type === "Combinada"
        ? legs.map((l) => l.eventName).filter(Boolean).join(" / ")
        : legs[0].eventName || null;
    const pick = legs.map((l) => l.pick).join(" / ");

    // Total odds: outer .hisCof block
    const oddsMatch = block.match(/<div class="hisCof"[^>]*>\s*([0-9.,]+)\s*<\/div>/);
    const odds = oddsMatch ? parseFloat(oddsMatch[1].replace(",", ".")) : 0;
    if (!odds) continue;

    // Bottom summary row: stake + payout. Stake is the FIRST <td class="ce">X EUR</td>
    // after the last <td colspan="2"></td>.
    ...
    // Payout cell: the <td class="ce" style="..."> just before </table>
    ...
    if (/^no pagado$/i.test(payoutRaw) || payoutRaw === "" || /^EUR$/i.test(payoutRaw)) {
      ...
      if (payout == null) {
        ...
        if (Math.abs(net) < 0.005) {
          ...
          // Partial refund < stake → treat as half-loss
          result = "L";
          pnl = parseFloat(net.toFixed(2));
        }
      }
...
export const importFromHtml22Bet = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: { html: string }) => {
    if (!input || typeof input.html !== "string") {
      throw new Error("HTML inválido");
    }
    if (input.html.length > 20_000_000) {
      throw new Error("HTML demasiado grande (máx. 20 MB)");
    }
    return input;
  })
  .handler(async ({ data, context }) => {
    validate22BetHtml(data.html);
    const bets = parse22BetHtml(data.html);
    if (bets.length === 0) {
      throw new Error(
    ...
    for (let i = 0; i < toInsert.length; i += CHUNK) {
      ...
      const { error } = await context.supabase.from("bets").insert(chunk);
    ...
    if (inserted > 0) {
      const { data: tip } = await context.supabase
        .from("tipsters")
        .select("id")
        .eq("name", "Sin asignar")
        .maybeSingle();
      if (!tip) {
        await context.supabase
          .from("tipsters")
          .insert({ user_id: context.userId, name: "Sin asignar", active: true });
      }
    }
